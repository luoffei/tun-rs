    ...
    pub fn try_read(&mut self, mut buf: &mut [u8]) -> io::Result<usize> {
        ...
        let result = if inner.no_pending_io {
            ...
            let result = ffi::try_read_file(
                HANDLE(inner.file_handle.as_raw_handle()),
            ...
            ffi::try_io_overlapped(HANDLE(inner.file_handle.as_raw_handle()), &inner.overlapped)
                .map(|size| size as _)
        };
        match result {
            Ok(len) => {
    ...
    pub fn try_write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        loop {
            return if inner.no_pending_io {
                ...
                let result = ffi::try_io_overlapped(
                    HANDLE(inner.file_handle.as_raw_handle()),
                    &inner.overlapped,
                )
                .map(|size| size as _);
...
impl Drop for OwnedOVERLAPPED {
    fn drop(&mut self) {
        if !self.no_pending_io {
            _ = ffi::cancel_io_overlapped(
                HANDLE(self.file_handle.as_raw_handle()),
                self.as_overlapped(),
            );
        }
    }
